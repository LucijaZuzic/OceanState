...
for first_part in starts:
    ...
    kernel = gaussian_kde(ang[first_part]["train"] + ang[first_part]["test"] + ang[first_part]["valid"]) 
    ...
    plt.hist([ang[first_part]["train"], ang[first_part]["test"], ang[first_part]["valid"]], bins = 360, stacked = True)

    for val in mini:
        plt.axvline(xx[val])
 
    plt.plot(xx, yxx * len(ang[first_part]["train"] + ang[first_part]["test"] + ang[first_part]["valid"])) 
    plt.show()
    plt.close()
