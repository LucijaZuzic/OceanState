                    ...
                    for k, long in enumerate(file2read.variables["longitude"]):
                        if long >= dict_xval_loc[loc][0] and long <= dict_xval_loc[loc][-1] and lat >= dict_yval_loc[loc][0] and lat <= dict_yval_loc[loc][-1]:
                            ...
                            if min(wave_heights) != max(wave_heights) and not np.isnan(min(wave_heights)):
                                ...
                                make_circles(np.arange(min_r, min_r + max(list(sum_in_b.values())) + step_size_use, step_size_use), np.arange(size_of_bin / 2, 360, size_of_bin), scf, xoff, yoff)
                                
                                plt.xlim(- (xoff + min_r + max(list(sum_in_b.values())) + step_size_use * 3), xoff + min_r + max(list(sum_in_b.values())) + step_size_use * 3)
                                plt.ylim(- (yoff + min_r + max(list(sum_in_b.values())) + step_size_use * 3), yoff + min_r + max(list(sum_in_b.values())) + step_size_use * 3)
 
                                plt.xticks([], [])
                                plt.yticks(rsss_all, rsss_tags_all)
                                plt.ylabel("Directional Distribution of Wave Heights (%)")

                                ax = plt.gca()
                                ax.set_aspect('equal', adjustable = 'box')

                                rcs = random_colors(len(wave_bins) + 1) 
                                lu = {wb: False for wb in wave_bins_for_bin[list(wave_bins_for_bin.keys())[0]]} 
                                ...
                                for b in dict(sorted(wave_bins_for_bin.items(), key=lambda item: item[0], reverse = True)):
                                    ...
                                    for ci, wb in enumerate(dict(sorted(wave_bins_for_bin[b].items(), key=lambda item: item[0]))): 
                                        ...
                                        if sum_r_new != sum_r:
                                            ...
                                            plot_arc(lnam, sum_r_new, sum_r, b[0], b[1], color_use = rcs[ci], draw_edges = True, scaling_factor = scf, xoffset = xoff, yoffset = yoff, use_label = not lu[wb])
                                            lu[wb] = True
                                        sum_r = sum_r_new
                                ...
                                plt.legend(loc = "lower center", bbox_to_anchor = (0.45, -0.23), ncol = len(lu) / 2, title = "Wave Height (m)")
                                plt.savefig("wind_rose/" + str(loc) + "/" + str(loc) + "_long_" + str(long) + "_lat_" + str(lat) + ".png", bbox_inches = "tight") 
                                plt.close()

                                print("wind_rose/" + str(loc) + "/" + str(loc) + "_long_" + str(long) + "_lat_" + str(lat) + ".png")
